...
from climateset.download.cmip6_downloader import CMIP6Downloader, CMIP6DownloaderV2
from climateset.download.input4mips_downloader import (
    Input4MipsDownloader,
    Input4MipsDownloaderV2,
)
...
@app.command(
    name="download-basic-v2",
    help="Download ClimateSet data by building the config objects. See function content for more details.",
)
def basic_download_v2():
    """
    By default, will download to the DATA_DIR folder. You can override this behavior modifying the config objects or by
    adding the `data_dir` key in the config file under each project.
    ...
    input4mips_downloader = Input4MipsDownloaderV2(input4mips_config)
    input4mips_downloader.download()

    cmip6_downloader = CMIP6DownloaderV2(cmip6_config)
    cmip6_downloader.download()
