...
class Input4MipsDownloader(AbstractDownloader):
    ...
    def download_meta_historic_biomassburning_single_var(
        ...
        self.logger.info(f"{variable}, {variable_id}, {institution_id}")
...
class Input4MipsDownloaderV2(AbstractDownloader):
    def __init__(self, config: Input4mipsDownloaderConfig, distrib: bool = True):
        self.config: Input4mipsDownloaderConfig = config
...
def input4mips_download_from_config(config):
    ...
    downloader = Input4MipsDownloaderV2(config=config_object)
    downloader.download()
